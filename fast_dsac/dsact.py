...
def main():
    ...
    if args.agent == "fasttd3":
        from dsact_net import Actor, Critic
    ...
    act_target = actor_cls(**actor_kwargs)
    act_target.load_state_dict(actor.state_dict())
    print()
    qnet = critic_cls(**critic_kwargs)
    qnet_target = critic_cls(**critic_kwargs)
    qnet_target.load_state_dict(qnet.state_dict())
    log_alpha = nn.Parameter(torch.log(torch.tensor(0.0001, device=device)))
    ...
    def update_main(data, meanstd, logs_dict):
        with autocast(
            ...
            with torch.no_grad():
                next_state_actions, next_log_prob = act_target(next_observations)
            with torch.no_grad():
            ...
            with torch.no_grad():
                ...
                if args.use_cdq:
                    ...
                    target_q1, target_q1_bound = compute_target_q(
                        ...
                        qf1.detach(),
                        mean_std1.detach(),
                        qf1_target_next_value.detach(),
                        qf1_target_next_value_sample.detach(),
                        next_log_prob.detach(),
                    )
                    target_q2, target_q2_bound = compute_target_q(
                        rewards,
                        ...
                        qf2.detach(),
                        mean_std2.detach(),
                        qf2_target_next_value.detach(),
                        qf2_target_next_value_sample.detach(),
                        next_log_prob.detach(),
                    )
                ...
                bias = args.bias
            ratio1 = (
                torch.pow(mean_std1, 2) / (torch.pow(qf1_std_detach, 2) + bias)
            ).clamp(min=0.1, max=10)
            ratio2 = (
                torch.pow(mean_std2, 2) / (torch.pow(qf2_std_detach, 2) + bias)
            ).clamp(min=0.1, max=10)
            qf1_loss = torch.mean(
                ratio1
                * (
                    huber_loss(qf1, target_q1, delta=50, reduction="none")
                    + qf1_std
                    * (
                        qf1_std_detach.pow(2)
                        - huber_loss(
                            qf1.detach(), target_q1_bound, delta=50, reduction="none"
                        )
                    )
                    / (qf1_std_detach + bias)
                )
            ...
            qf2_loss = torch.mean(
                ratio2
                * (
                    huber_loss(qf2, target_q2, delta=50, reduction="none")
                    + qf2_std
                    * (
                        qf2_std_detach.pow(2)
                        - huber_loss(
                            qf2.detach(), target_q2_bound, delta=50, reduction="none"
                        )
                    )
                    / (qf2_std_detach + bias)
                )
        ...
        logs_dict["mean_std1"] = mean_std1.max().detach()
        logs_dict["mean_std2"] = mean_std2.min().detach()
        return logs_dict
    ...
    while global_step < args.total_timesteps:
        ...
        with torch.no_grad(), autocast(
            ...
            actions, log_probs = actor(obs=norm_obs)
            ...
            for i in range(args.num_updates):
                ...
                if args.num_updates > 1:
                    if i % args.policy_frequency == 1:
                        ...
                        soft_update(actor, act_target, args.tau)
            ...
            if global_step % 100 == 0 and start_time is not None:
                ...
                with torch.no_grad():
                    logs = {
                        ...
                        "env_rewards_max": rewards.max(),
                        "env_rewards_min": rewards.min(),
